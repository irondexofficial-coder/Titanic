...
def PredictionFunction(Pclass,Sex,Age,SibSp,Parch,Fare,Embarked):
    try:
        ...
        print('Something went wrong. Contact IT support at +434949494.')
...
st.sidebar.info('Example: A 30 years old male, 1st class, $100 fare, traveling alone from port Southempton.')
...
def main():
    ...
    with col2:
        ...
        Fare = st.slider('Fare($):', min_value=0.0, max_value=500.0, step=0.01)
        Embarked = st.radio('Port of Embarkation:', options=['C','Q','S'])
            #already numerical so no conversion has to be done 
    ...
    if st.button('Predict'):
        result = PredictionFunction(Pclass, Sex, Age, SibSp, Parch, Fare, Embarked) #result holds 0 or 1 
        st.info(f'{result}')
